...
- Dependance LÖVE obligatoire: installation stricte dans le bootstrap, avec contournement automatique Debian 11 si le paquet `love` casse sa post-installation, sans confondre un autre echec `apt` avec `love`.
- Runtime Python Raspberry Pi durci: `python3-pygame` est installe au niveau systeme pour recuperer les bibliotheques SDL minimales, et `pip` ajoute `piwheels` comme index supplementaire de secours sur Raspberry Pi OS.
...
- Garde anti-regression installation: `scripts/tests/test_installation.sh` verifie maintenant aussi que les scripts critiques restent executables apres installation (`bootstrap_borne.sh`, `scripts/deploiement/post_pull_update.sh`, `.githooks/post-merge`, `./borne_arcade/lancerBorne.sh`) et que des dossiers/ressources non executables restent partageables pour tous (`docs/`, `config/`, images borne).
